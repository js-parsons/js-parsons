...
LIS.patience_sort = function(list) {
  var arr = my_.toArray(list),
      decks = [[arr[0]]],
      deckPos = 0;
  for (var i = 1; i < arr.length; i++) {
    var x = arr[i],
        currDeck = decks[deckPos];
    if (x < my_.last(currDeck)) {
...
LIS.find_lises = function(decks) {
  ...
  lis_length = my_.max(my_.map(lises, function(item) { return item.length; }));
  lises = my_.select(lises, function(item) { return item.length >= lis_length; });
  return lises;
}

LIS.best_lise = function(lises) {
  var lis_scores = my_.map(lises, function(item, index) {
    if (item.length <= 1) {
  ...
  var best = my_.max(lis_scores, function(item) { return item.score });
  return lises[best.index];
}

LIS.best_lise_inverse = function(list) {
  var decks = this.patience_sort(my_.toArray(list)),
      lises = this.find_lises(decks)
      best = this.best_lise(lises);
  return my_.difference(list, best);
}
...
function lis_inverses(list) {
  var inverse_list = [],
      decks = patience_sort(my_.toArray(list)),
      lises = find_lises(decks);
  return my_.map(lises, function(item) { return my_.difference(list, item); });
}

function in_all_lis_inverses(list) {
  var inverse_list = lis_inverses(list);
  return my_.intersection.apply(null, inverse_list);
}

function inverse_indices(list) {
  var in_all = in_all_lis_inverses(list);
  return my_.map(in_all, function(item) { return list.indexOf(item); });
};*/
